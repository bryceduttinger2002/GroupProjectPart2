...
def read_one(db: Session, review_id):  # Updated to review_id
    try:
...
def update(db: Session, review_id, request):  # Updated to review_id
    try:
...
def delete(db: Session, review_id):  # Updated to review_id
    try:
        review = db.query(model.Review).filter(model.Review.id == review_id)
        if not review.first():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Review not found!")
